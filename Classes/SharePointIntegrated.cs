...
namespace RSS_Report_Retrievers
{
    class SharePointIntegrated : IReportingServicesFactory
    {
        ...
        public SharePointIntegrated(TreeView treeView, ToolStripStatusLabel label, ListView listView)
        {
        ...
        /// <summary>
        /// In SharePoint mode there is no option to recursively get the catalogitems on the server.
        /// This methods provides an alternative.
        /// </summary>
        /// <param name="path">path to start query</param>
        /// <param name="recursive">get catalogitems recursively</param> 
        /// <returns>List of all catalogitems at the given path</returns>
        private List<CatalogItem> GetCatalogItems(string path, bool recursive)
        {
            List<CatalogItem> catalogItems = new List<CatalogItem>();

            foreach (CatalogItem item in rs.ListChildren(path))
            {
                catalogItems.Add(item);
                if (item.Type == ItemTypeEnum.Folder && recursive)
                {
                    foreach (CatalogItem subitem in GetCatalogItems(item.Path, recursive))
                    {
                        catalogItems.Add(subitem);  
                    }
                }
            }
        ...
        {
            foreach (CatalogItem item in GetCatalogItems(parent.ToolTipText, false))
            {
        ...
        public void PopulateItems(string path)
        {
            ...
            foreach (CatalogItem item in GetCatalogItems(path, false))
            {
        ...
        public void UploadFolder(string path, string destination, bool overwrite, TreeNode parent)
        {
            ...
            {
                TreeNode newFolderNode = CreateFolder(Path.GetFileName(foldername), parent);
                UploadFolder(foldername, destination + "/" + Path.GetFileName(foldername), overwrite, newFolderNode);
            }
        }
            ...
            switch (type)
            {
                ...
                case ItemTypes.Report:
                    try
                    {
                        ...
                        definition.Save(String.Format(@"{0}\{1}", directory, filename));
