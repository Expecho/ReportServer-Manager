...
using System.IO;
using System.Linq;
using ReportingServerManager.RSS_2008R2;
...
namespace ReportingServerManager.Logic
{
    ...
    {
        private readonly ReportingService2010 webserviceProxy = new ReportingService2010();
        ...
        public ReportWarning[] CreateModel(string visibleName, string parentFolder, Byte[] definition, string properties)
        {
            ...
            webserviceProxy.CreateCatalogItem("Model", visibleName, parentFolder, true, definition, null, out warnings);
        ...
        {
            return webserviceProxy.GetItemDefinition(path);
        }
        ...
        {
            return webserviceProxy.GetItemParameters(path, null, false, null, null)
                .Select(parameter => new List<String>
                            {
                                parameter.Name, parameter.ParameterTypeName, parameter.AllowBlank.ToString(), parameter.Nullable.ToString(), parameter.MultiValue.ToString(), parameter.Prompt
                            })
                .ToList();
        }
        ...
        {
            return webserviceProxy.ListExtensions("Data")
        ...
        {
            return webserviceProxy.ListRoles("Catalog", SiteUrl).Select(r => r.Name);
        }
        ...
        private static ReportItemTypes GetReportItemTypeFromSSRSItemType(string ssrsType)
        {
            var convertedType = ReportItemTypes.Unknown;

            switch (ssrsType)
            {
                case "Folder": convertedType = ReportItemTypes.Folder; break;
        ...
        private static ReportItemDTO ConvertCatalogItemToReportItemDTO(CatalogItem item)
        {
            return new ReportItemDTO
            {
                ...
                Name = item.Name,
                Path = item.Path,
                Type = GetReportItemTypeFromSSRSItemType(item.TypeName)
            };
        }
